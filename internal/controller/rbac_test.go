...
// hasNoTypos checks for known typos in resource names.
func hasNoTypos(rules []rbacRule) []string {
	knownBadResources := map[string]string{
		"environmentsrequests":        "environmentrequests (extra 's')",
		"environmentsrequests/status": "environmentrequests/status (extra 's')",
	}
	var violations []string
	for _, rule := range rules {
		for _, r := range rule.Resources {
			if fix, bad := knownBadResources[r]; bad {
				violations = append(violations, fmt.Sprintf("resource '%s' looks like a typo, should be '%s'", r, fix))
			}
		}
	}
	return violations
...
var _ = Describe("RBAC Rules", func() {
	...
	for file, expectations := range controllerExpectations {
		...
		Context(fmt.Sprintf("in %s", file), func() {
			...
			It("should not have typos in resource names", func() {
				violations := hasNoTypos(rules)
				Expect(violations).To(BeEmpty(),
